...
import getSessionOptions from "./sessions/getSessionOptions.js";
...
function getExpressApplication(
  ...
  if (options.serving.favicon) {
    app.use(favicon(options.serving.favicon));
  }
  ...
  if (options.serving.static) {
    app.use("/static", serveStatic(options.serving.static));
  }
  ...
  app.use(cookieParser(options.cookies.secret));
  app.use(
    session(
      getSessionOptions(
        options.isLocal,
        options.sessions.secret || "development",
        options.sessions.redis.host || "localhost",
        options.sessions.redis.port || 9000
      )
    )
  );
